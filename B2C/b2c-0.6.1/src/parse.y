...
static int (*CreateForImp)(int string);
...
static int (*CreateAddressExp)(void);
...
void install2beta0(int i, int (*cb)(void))
{
    ...
    case 39: CreateAddressExp = cb; break;
    }
}
...
%token	PAT_BEG PAT_END ADDRESS PATTERNVAR ASSIGN CASE_SELECTION REFERENCE
%token	ENTER DO EXIT INNER LEAVE RESTART IF THEN ELSE FOR REPEAT
...
/* ForImp - a simple loop */
ForImp:
	'(' FOR aname ':' Evaluation REPEAT Imperatives FOR ')'
	    { (*CreateForImp)($3); free((char *) $3); }
	;
...
Factor:
	INTEGER { (*CreateIntegerConst)((int) (yytext)); }
	...
	| ADDRESS AttributeDenotation { (*CreateAddressExp)(); }
	| Transaction
	;
